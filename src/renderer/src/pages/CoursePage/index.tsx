...
interface AnnouncementCardProps { announcement: CourseMetadata['announcements'][number] }
function AnnouncementCard({ announcement }: AnnouncementCardProps) {
    return <Accordion disableGutters id={announcement.title}>
        ...
        <AccordionDetails>
            <Stack direction="row" spacing={1}>
                <Box flexGrow={1} />
                <Typography>
                    <Link to={`/user/${announcement.author.username}`}>
                        {announcement.author.full_name}
                    </Link>
                </Typography>
                <Stack direction="row">
                    <CommentIcon /><Typography>{announcement.comments}</Typography>
                </Stack>
                <Stack direction="row">
                    <VisibilityIcon /><Typography>{announcement.visits}</Typography>
                </Stack>
            </Stack>
...
interface TimeslotCardProps { timeslot: CourseMetadata['timeslots'][number] }
function TimeslotCard({ timeslot }: TimeslotCardProps) {
    return <Paper>
        <Stack direction="row">
            <Box sx={{ width: '20%' }}>{timeslot.description}</Box>
            <Box>
                {WEEKDAY_INDEX_TO_WEEKDAY[timeslot.day]}, {timeslot.start_time.hour}:{timeslot.start_time.minute.toString().padStart(2, '0')} - {timeslot.end_time.hour}:{timeslot.end_time.minute.toString().padStart(2, '0')}
            </Box>
            <Box flexGrow={1} />
            {timeslot.locations.length > 0 && (
                <Stack direction="row" spacing={1}>
                    {timeslot.locations.map((location) => {
                        if (location.id)
                            return <Link key={location.id} to={`/location/${location.id}`}>{location.name}</Link>
...
interface FileWidgetProps { file: File, on_file_click: (file: File) => void }
...
interface FolderWidgetProps { folder: Folder, on_folder_click: (label: string, contents: Folder['contents']) => void }
...
interface CourseFilesWidgetProps { files: Folder['contents'] | false | undefined }
function CourseFilesWidget({ files }: CourseFilesWidgetProps) {
    ...
    const on_folder_click = useCallback((label: string, files: Folder['contents']) => {
        set_selected_files_stack([...selected_files_stack, { label, files }])
    }, [selected_files_stack])

    const on_file_click = useCallback((file: File) => {
        IPC.download_file(file.name, file.download_url)
    }, [])

    const on_history_click = useCallback((index: number) => {
    ...
    return <Paper>
        <Stack direction="row">
            ...
            <Stack sx={{ marginLeft: 1, marginBottom: 2 }} direction="row">
                {selected_files_stack
                    .map<React.ReactNode>(({ label}, index) => <Box key={index} sx={{ cursor: 'pointer' }} onClick={() => on_history_click(index)}>{label}</Box>)
                    .reduce((prev, curr) => [prev, ' / ', curr])
                }
            </Stack>
        </Stack>
        <Stack>
            {currently_displayed_filelist.folders.map((folder) => <FolderWidget
                key={folder.id} folder={folder} on_folder_click={on_folder_click}
            />)}
            {currently_displayed_filelist.files.map((file) => <FileWidget
                key={file.id} file={file} on_file_click={on_file_click}
            />)}
        </Stack>
    </Paper>
}
...
function CoursePage() {
    ...
    useEffect(() => {
        if (course_info === undefined)
            useUserState.getState().fetch_course_metadata(course_id)
    }, [course_info]);
    ...
    if (course_info === false)
        return <>Failed to fetch Course Info</>
    ...
    return <Box sx={{ paddingLeft: 3, paddingRight: 3 }}>
        ...
        {(course_info.announcements.length > 0) && (
            <>
                <Typography variant="h6" sx={{ marginTop: 3 }}>Announcements:</Typography>
                {course_info.announcements.map((announcement, index) => <AnnouncementCard announcement={announcement} key={index} />)}
            </>
        )}
        <Typography variant="h6" sx={{ marginTop: 3 }}>Timeslots:</Typography>
        <Stack spacing={1}>
            {course_info.timeslots.map((timeslot, index) => <TimeslotCard timeslot={timeslot} key={index} />)}
        </Stack>
        {(course_info.supports.files) && (
            <>
                <Typography variant="h6" sx={{ marginTop: 3 }}>Files:</Typography>
